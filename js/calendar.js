...
async function renderSlots() {
  ...
  const todayStr=today(), monthLabel=`${MONTH_RU[monday.getMonth()]} ${monday.getFullYear()}`;
  ...
  const renderCell=(d,t)=>{
    ...
    if(slot&&slot.booked_by==='ervin'){
      ...
      return`<div class="cal-pill cal-pill-ervin" ${can?`onclick="unbookErvin('${slot.id}')" title="Снять бронь"`:'title="Занято Ervin"'}>
        <div class="cal-pill-name">Ervin</div><div class="cal-pill-sub">${slot.ervin_note||''}</div></div>`;}
    if(slot&&!slot.is_booked){
      if(isErvin()) return`<div class="cal-pill cal-pill-free" onclick="bookErvinAt('${d}','${t}','${slot.id}')"><div class="cal-pill-name">Свободно</div><div class="cal-pill-sub">занять</div></div>`;
      if(isAdmin()) return`<div class="cal-pill cal-pill-free" onclick="removeSlot('${slot.id}')"><div class="cal-pill-name">✓ Открыт</div><div class="cal-pill-sub">убрать</div></div>`;
  ...
  <div class="cal-grid">
    ...
    ${weekDays.map(d=>{const dt=new Date(d+'T12:00:00');const isToday=d===todayStr;const isWork=workDays.includes(dt.getDay());
      return`<div class="cal-grid-head${isToday?' today':''}" style="${!isWork?'opacity:.45':''}">
        ...
        ${isAdmin()?`<button style="font-size:10px;padding:1px 5px;border:1px solid var(--border);border-radius:4px;background:white;cursor:pointer;color:var(--text-m);margin-top:3px" onclick="openDaySlots('${d}')">+ День</button>`:''}
      </div>`;}).join('')}
    ${allTimes.map(t=>`
  ...
  <div style="display:flex;gap:16px;margin-top:12px;font-size:12px;color:var(--text-m);flex-wrap:wrap">
    <span style="display:flex;align-items:center;gap:4px"><span style="width:11px;height:11px;border-radius:3px;background:#d1fae5;display:inline-block"></span>Открыт</span>
    <span style="display:flex;align-items:center;gap:4px"><span style="width:11px;height:11px;border-radius:3px;background:#dbeafe;display:inline-block"></span>Пациент</span>
    <span style="display:flex;align-items:center;gap:4px"><span style="width:11px;height:11px;border-radius:3px;background:#fde68a;display:inline-block"></span>Ervin</span>
    ${isAdmin()?`· <a href="#" onclick="nav('settings')" style="color:var(--accent);font-size:12px">Настройки расписания</a>`:''}
  </div>`;
}
...
async function openDaySlots(date){
  ...
  openModal(`<div class="modal"><div class="modal-header"><span class="modal-title">Открыть слоты на день</span><button class="btn btn-ghost btn-sm" onclick="closeModal()">✕</button></div>
    ...
    <div class="modal-footer"><button class="btn btn-ghost" onclick="closeModal()">Отмена</button><button class="btn btn-accent" onclick="saveDaySlots()">Открыть</button></div>
  </div>`);
}
...
async function saveDaySlots(){
  ...
  toast(`Открыто ${rows.length} слотов`);closeModal();renderSlots();
}
...
async function openWeekSlots(from,to){
  ...
  openModal(`<div class="modal"><div class="modal-header"><span class="modal-title">Открыть неделю</span><button class="btn btn-ghost btn-sm" onclick="closeModal()">✕</button></div>
    ...
    <div class="modal-footer"><button class="btn btn-ghost" onclick="closeModal()">Отмена</button><button class="btn btn-accent" onclick="saveWeekSlots()">Открыть</button></div>
  </div>`);
}
...
async function saveCustomSlot(){
  ...
  if(!date||!time)return;
  await db.from('available_slots').upsert({date,start_time:time,is_booked:false,booked_by:null},{onConflict:'date,start_time'});
  toast('Слот добавлен');closeModal();renderSlots();
}
...
async function bookErvinAt(date,time,slotId){
  openModal(`<div class="modal"><div class="modal-header"><span class="modal-title">Запись Ervin — ${time} ${fmt(date)}</span><button class="btn btn-ghost btn-sm" onclick="closeModal()">✕</button></div>
    ...
    <div class="modal-footer"><button class="btn btn-ghost" onclick="closeModal()">Отмена</button><button class="btn btn-accent" onclick="_confirmErvinBook('${date}','${time}','${slotId||''}')">Занять</button></div>
  </div>`);
}
...
async function unbookErvin(id){if(!confirm('Снять бронь Ervin?'))return;await db.from('available_slots').delete().eq('id',id);renderSlots();}
...
async function openAddErvinBooking(){
  openModal(`<div class="modal"><div class="modal-header"><span class="modal-title">Моя запись</span><button class="btn btn-ghost btn-sm" onclick="closeModal()">✕</button></div>
    ...
    <div class="modal-footer"><button class="btn btn-ghost" onclick="closeModal()">Отмена</button><button class="btn btn-accent" onclick="_saveErvinBooking()">Занять</button></div>
  </div>`);
}
...
async function _saveErvinBooking(){
  ...
  toast('Запись добавлена');closeModal();renderSlots();
}
