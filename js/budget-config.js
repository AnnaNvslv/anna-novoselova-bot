...
let bdb=null;
try{const{createClient}=window.supabase;bdb=createClient(SB_URL,SB_KEY);}catch(e){console.error(e);}
...
const DEFAULT_CATEGORIES=['Жильё','Садик','Занятия детские','Животные','Машина','Связь','Транспорт','Подписки','Аня','Ваня Концерта','Продукты','Дом','Другое'];
function getCategories(){return[...new Set([...DEFAULT_CATEGORIES,..._customCategories])].sort();}
function toDin(amount,currency){
  if(!amount)return 0;const n=parseFloat(amount)||0;
...
const INCOME_SOURCES=[
  ...
  {id:'balance_start',label:'Остаток (начало мес.)',icon:'💰',auto:false},
  {id:'other',      label:'Другое',      icon:'➕',auto:false},
];
const EXPENSE_STATUS={
  planned: {label:'Запланировано',color:'#b8a8a0'},
  reserved:{label:'Отложено',     color:'#d4946a'},
  paid:    {label:'Оплачено',     color:'#6dab8a'},
};
...
async function loadRates(){
  ...
  _customCategories=s.custom_categories?JSON.parse(s.custom_categories):[];
}
...
async function saveCustomCategories(){await bdb.from('budget_settings').upsert({key:'custom_categories',value:JSON.stringify(_customCategories)});}
