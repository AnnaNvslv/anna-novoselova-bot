...
async function renderPatients() {
  document.getElementById('content').innerHTML=`<div class="topbar"><h1>Пациенты</h1><div class="topbar-actions"><div class="search-wrap"><input type="text" id="psearch" placeholder="Поиск..." oninput="filterPatientsUI(this.value)"></div><button class="btn btn-accent" onclick="openAddPatient()">+ Пациент</button></div></div><div class="content"><div class="spinner">Загрузка...</div></div>`;
...
function renderPatientsTable(patients) {
  ...
  c.innerHTML=`<div class="card"><div class="table-wrap"><table>
    <thead><tr><th>Пациент</th><th>Телефон</th><th>Telegram</th><th>Возраст</th><th></th></tr></thead>
    <tbody>${patients.map(p=>`<tr id="prow-${p.id}" style="${_lastAddedPatientId===p.id?'background:#d1fae5;transition:background 2s':''}">
      ...
      <td><div class="flex gap-8">
        ...
        <button class="btn btn-accent btn-sm" onclick="openPatientCard('${p.id}')">Карточка</button>
    ...
    </tr>`).join('')||`<tr><td colspan="5"><div class="empty"><p>Нет пациентов</p></div></td></tr>`}
    </tbody></table></div></div>`;
    ...
    <div class="modal modal-full">
      ...
      <div class="modal-body">
        <div class="info-grid mb-12">
          <div class="info-item"><label>Телефон</label><p>${p.phone||'—'}</p></div>
          ...
          <div class="info-item"><label>Дата рождения</label><p>${p.dob?fmt(p.dob)+(age?' ('+age+' лет)':''):'—'}</p></div>
          <div class="info-item"><label>Откуда узнал</label><p>${p.source||'—'}</p></div>
          <div class="info-item"><label>В базе с</label><p>${fmt(p.created_at?.split('T')[0])}</p></div>
        </div>
        ${p.notes?`<div class="mb-12 text-m text-sm">${p.notes}</div>`:''}
        <div class="divider"></div>
        <div class="tab-bar">
          <div class="tab${_cardTab==='appts'?' active':''}" onclick="_cardTab='appts';_renderPatientCard('${pid}')">Приёмы (${(appts||[]).length})</div>
          <div class="tab${_cardTab==='exams'?' active':''}" onclick="_cardTab='exams';_renderPatientCard('${pid}')">Карты (${(exams||[]).length})</div>
          <div class="tab${_cardTab==='orders'?' active':''}" onclick="_cardTab='orders';_renderPatientCard('${pid}')">Заказы (${(orders||[]).length})</div>
        </div>
...
function _apptTab(appts,pid){
  if(!appts.length)return`<div class="empty"><p>Нет приёмов</p></div>`;
  return appts.map(a=>`<div class="history-item">
    ...
    <div style="flex:1">
      ...
      ${a.consultation_price?`<div class="text-sm text-m">Стоимость: ${fmtMoney(a.consultation_price)}</div>`:''}
    </div>
    <div class="flex gap-8">
      ...
      ${a.status==='запланирован'?`<button class="btn btn-success btn-sm" onclick="confirmCompleteAppt('${a.id}')">✓</button><button class="btn btn-danger btn-sm" onclick="cancelAppt('${a.id}')">Отменить</button>`:''}
      ${a.status==='отменён'?`<span class="badge badge-gray">Отменён</span>`:''}
    </div>
  </div>`).join('');
}
function _examTabHtml(exams,pid){
  if(!exams.length)return`<div class="empty"><p>Нет карт обследования</p></div>`;
  return exams.map(e=>`<div class="history-item">
    ...
    <div class="flex gap-8">
      <button class="btn btn-ghost btn-sm" onclick="openExamView('${e.id}','${pid}')">Открыть</button>
      <button class="btn btn-primary btn-sm" onclick="printExam('${e.id}')">🖨️</button>
    </div>
  </div>`).join('');
}
function _orderTab(orders,pid){
  if(!orders.length)return`<div class="empty"><p>Нет заказов</p></div>`;
  return orders.map(o=>{const bal=orderBalance(o);return`<div class="history-item">
    ...
    <div class="flex gap-8">
      ...
      <button class="btn btn-ghost btn-sm" onclick="openOrderCard('${o.id}')">Карточка</button>
    </div>
  </div>`;}).join('');
}
...
function _patientForm(p){
  openModal(`<div class="modal modal-lg">
    <div class="modal-header"><span class="modal-title">${p?'Редактировать пациента':'Новый пациент'}</span><button class="btn btn-ghost btn-sm" onclick="closeModal()">✕</button></div>
    <div class="modal-body">
      <div class="form-grid">
        <div class="form-group full"><label>ФИО *</label><input id="p-name" value="${p?.name||''}" placeholder="Иванова Мария Петровна"></div>
        <div class="form-group"><label>Телефон</label><input id="p-phone" value="${p?.phone||''}"></div>
        ...
        <div class="form-group full"><label>Откуда узнал</label>
          <select id="p-source"><option value="">— выберите —</option>${SOURCES.map(s=>`<option ${p?.source===s?'selected':''}>${s}</option>`).join('')}</select>
        </div>
        <div class="form-group full"><label>Заметки</label><textarea id="p-notes">${p?.notes||''}</textarea></div>
      </div>
    </div>
    <div class="modal-footer">
      <button class="btn btn-ghost" onclick="closeModal()">Отмена</button>
      <button class="btn btn-accent" onclick="savePatient('${p?.id||''}')">Сохранить</button>
    </div>
  </div>`);
}
...
async function savePatient(id){
  const name=v('p-name');if(!name){alert('Введите имя');return;}
  ...
  if(id){await db.from('patients').update(data).eq('id',id);toast('Обновлено');_lastAddedPatientId=null;}
  else{const{data:np}=await db.from('patients').insert(data).select().single();_lastAddedPatientId=np?.id;toast('Добавлен');}
  closeModal();render();
}
async function delPatient(id){if(!confirm('Удалить пациента со всеми данными?'))return;await db.from('patients').delete().eq('id',id);toast('Удалено');render();}
